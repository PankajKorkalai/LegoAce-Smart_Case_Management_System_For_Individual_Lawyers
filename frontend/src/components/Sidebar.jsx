...
export default function Sidebar({ collapsed, onCollapsedChange, mobileOpen, onMobileClose }) {
  return (
    <>
      {/* Backdrop for mobile */}
      {mobileOpen && (
      ...
      <div
        className={`fixed left-0 top-0 z-[70] flex h-screen flex-col justify-between overflow-hidden border-r border-gray-200/70 bg-gray-50/90 backdrop-blur-xl backdrop-saturate-150 transition-all duration-300 ease-in-out ${
          collapsed ? "w-20" : "w-64"
        } ${
      ...
      >
        ...
        <div className="flex flex-col h-full overflow-hidden">
          ...
          >
            ...
            <button
              type="button"
              className="shrink-0 p-1.5 rounded-lg hover:bg-gray-200 text-gray-600 transition-colors hidden lg:flex"
            ...
            {/* Mobile close button inside the sidebar (optional but nice) */}
            <button 
                ...
                >
                  ...
                  <span 
                    className={`whitespace-nowrap overflow-hidden transition-all duration-300 ease-in-out ${
                      collapsed ? "lg:max-w-0 lg:opacity-0 ml-0" : "max-w-[200px] opacity-100 ml-3"
                    }`}
                  >
                    {item.name}
                  </span>
